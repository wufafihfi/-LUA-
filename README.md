...
## 交互相关function
```lua
bzdAPI.GetKeyPress_oneR(<虚拟键码>0x01,<检测模式,0为窗口活动才检测按键,1为程序可以检测到任何时候的按键>0) --检测按键按下（按下按键只返回一次true,需要松开按键才能继续）
bzdAPI.GetKeyPress(<虚拟键码>0x01,<检测模式,0为窗口活动才检测按键,1为程序可以检测到任何时候的按键>0) --检测按键按下（按下按键则返回true,松开则为false）
```
## 数据获取相关function
```lua
